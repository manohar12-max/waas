...
export default function MediaFeedPage() {
  ...
  const handleLike = async (postId: string) => {
    try {
      const token = localStorage.getItem('token');
      const res = await axios.post(`${import.meta.env.VITE_API_URL}/media-feed/${postId}/like`, {}, {
        headers: { Authorization: `Bearer ${token}` }
      });
        ...
        <AnimatePresence mode="popLayout">
          {filteredAndSortedPosts.map((post, idx) => (
            <motion.div key={post._id} layout initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.1 }} className="group bg-card-light dark:bg-card-dark border border-slate-200 dark:border-white/10 rounded-[48px] overflow-hidden shadow-2xl hover:border-primary-light/30 transition-all active:scale-[0.99]">
              <div className="max-h-[600px] min-h-[300px] relative overflow-hidden bg-slate-900 flex items-center justify-center p-2 pb-6">
                  ...
                  <div className="flex items-center justify-between pt-4 border-t border-slate-200 dark:border-white/5">
                    <div className="flex items-center gap-4">
                        <button 
                          ...
                          className={`flex items-center gap-2 text-[10px] font-black uppercase tracking-widest transition-all p-2 rounded-lg ${post.likes?.includes(user._id) ? 'text-red-500 bg-red-500/10' : 'text-slate-400 hover:text-red-500 hover:bg-red-500/5'}`}
                        >
