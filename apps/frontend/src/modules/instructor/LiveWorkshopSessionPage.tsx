      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
        ...
        <div className="lg:col-span-4 space-y-10">
          <div className="bg-card-light dark:bg-card-dark border border-slate-200 dark:border-white/5 rounded-[56px] p-10 shadow-2xl space-y-8">
            ...
            <div className="space-y-6"><div><div className="flex justify-between items-end mb-2"><span className="text-[10px] font-black uppercase tracking-widest opacity-40">Participation Rate</span><span className="text-sm font-black text-primary-light">{workshop?.registeredStudentIds?.length ? Math.min(100, Math.round((attendance.filter((a: any) => a.status === 'PRESENT').length / workshop.registeredStudentIds.length) * 100)) : 0}%</span></div><div className="h-2 bg-white/5 rounded-full overflow-hidden"><motion.div initial={{ width: 0 }} animate={{ width: `${workshop?.registeredStudentIds?.length ? Math.min(100, (attendance.filter((a: any) => a.status === 'PRESENT').length / workshop.registeredStudentIds.length) * 100) : 0}%` }} className="h-full bg-primary-light" /></div></div></div>
          </div>
