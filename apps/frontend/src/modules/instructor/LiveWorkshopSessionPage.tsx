                    ...
                    <tbody className="divide-y divide-white/5">
                      {attendance.map((record) => (
                        <tr key={record._id} className="group hover:bg-white/5 transition-colors">
                          ...
                          <td className="px-10 py-6">{(user.role === 'TEACHER' || user.role === 'COLLEGE_ADMIN') && (<button onClick={() => handleOverride(record._id, record.status === 'PRESENT' ? 'ABSENT' : 'PRESENT')} className="p-2 hover:bg-primary-light/10 text-primary-light rounded-lg transition-all cursor-pointer" title="Override Status"><Activity className="w-4 h-4" /></button>)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
