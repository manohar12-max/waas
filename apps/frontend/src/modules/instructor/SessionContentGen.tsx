...
export default function SessionContentGen({ workshop, workshopId, onUpdate }: { workshop?: any, workshopId: string, onUpdate?: () => void }) {
  ...
  // Ref to prevent infinite auto-generation if user explicitly deletes all days
  ...
  const fetchStructure = async () => {
    try {
      ...
      let currentDays = res.data;

      // Only perform auto-generation check on the first fetch
      if (!hasAutoGeneratedRef.current) {
        hasAutoGeneratedRef.current = true; // Mark as checked
        
        if (workshop?.schedule?.start && workshop?.schedule?.end) {
          const startDate = new Date(workshop.schedule.start);
          const endDate = new Date(workshop.schedule.end);
          const diffDays = Math.max(1, Math.ceil((endDate.getTime() - startDate.getTime()) / (1000 * 3600 * 24)) + 1);
          
          // Back-fill if there are fewer days in DB than the event duration
          if (currentDays.length < diffDays) {
            try {
              const daysToAdd = diffDays - currentDays.length;
              const lastDayNum = currentDays.length > 0 ? currentDays[currentDays.length - 1].dayNumber : 0;
              const lastDate = currentDays.length > 0 ? new Date(currentDays[currentDays.length - 1].date) : new Date(startDate);
              if (currentDays.length > 0) lastDate.setDate(lastDate.getDate() + 1);

              for (let i = 0; i < daysToAdd; i++) {
                const currDate = new Date(lastDate);
                currDate.setDate(lastDate.getDate() + i);
                await axios.post(`${import.meta.env.VITE_API_URL}/sessions-content/days`, { 
                  workshopId, 
                  dayNumber: lastDayNum + i + 1,
                  date: currDate.toISOString().split('T')[0]
                }, { headers: { Authorization: `Bearer ${token}` } });
              }
              // Refetch after creation
              const newRes = await axios.get(`${import.meta.env.VITE_API_URL}/sessions-content/workshop/${workshopId}`, {
                headers: { Authorization: `Bearer ${token}` }
              });
              currentDays = newRes.data;
            } catch (e) {
              console.error("Auto generation of days failed", e);
            }
          }
        }
      }
      ...
      setExpandedDays(prev => {
        if (prev.length === 0 && currentDays.length > 0) {
          return [currentDays[0]._id];
        }
        return prev;
      });
    } catch (err) {
  ...
  const handleAddDay = async () => {
    try {
      ...
      // Extend workshop end date by 1 day
      if (workshop?.schedule?.end) {
        const currentEnd = new Date(workshop.schedule.end);
        currentEnd.setDate(currentEnd.getDate() + 1);
        await axios.patch(`${import.meta.env.VITE_API_URL}/workshops/${workshopId}`, {
          schedule: {
            ...workshop.schedule,
  ...
  const handleDeleteDay = async (e: React.MouseEvent, dayId: string) => {
    ...
    try {
      ...
      // Decrease workshop end date by 1 day if we delete a day
      if (workshop?.schedule?.end && workshop?.schedule?.start) {
        const currentEnd = new Date(workshop.schedule.end);
        currentEnd.setDate(currentEnd.getDate() - 1);
        
        // ensure end date does not go before start date
        if (currentEnd >= new Date(workshop.schedule.start)) {
          await axios.patch(`${import.meta.env.VITE_API_URL}/workshops/${workshopId}`, {
            schedule: {
              ...workshop.schedule,
              end: currentEnd.toISOString()
  ...
  const startDate = workshop?.schedule?.start ? new Date(workshop.schedule.start) : null;
  const endDate = workshop?.schedule?.end ? new Date(workshop.schedule.end) : null;
  const totalWorkshopDays = startDate && endDate 
    ? Math.max(1, Math.ceil((endDate.getTime() - startDate.getTime()) / (1000 * 3600 * 24)) + 1)
    : days.length;
        ...
        <div>
          ...
          {startDate && endDate && (
            <p className="text-sm font-bold opacity-50 mt-1 uppercase tracking-widest">
              Workshop Duration: {totalWorkshopDays} {totalWorkshopDays === 1 ? 'Day' : 'Days'} 
              <span className="opacity-50 ml-2 lowercase">
        ...
        <button
          onClick={() => {
            if (days.length > 0) {
              ...
              setNewDay({ dayNumber: 1, date: startDate ? startDate.toISOString().split('T')[0] : new Date().toISOString().split('T')[0] });
            }
            ...
            <AnimatePresence>
              {expandedDays.includes(day._id) && (
                <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="px-8 pb-10 space-y-6">
                  ...
                  <button onClick={() => setShowAddSessionModal({ dayId: day._id })} className="w-full py-4 border-2 border-dashed border-primary-light/40 hover:border-primary-light rounded-3xl text-[10px] font-black uppercase hover:text-primary-light transition-all tracking-widest text-slate-500 dark:text-white/70">+ Add Session</button>
                </motion.div>
              )}
            </AnimatePresence>
          </div>
